@extends('layouts.app')
@section('title', 'RME — ' . ($visit->patient?->nama_lengkap ?? ''))
@section('breadcrumb')
    <a href="{{ route('rme.index') }}" class="hover:opacity-70 transition-opacity" style="color: #6B4C4C;">Rawat Jalan</a>
    <span style="color: #E8D5D5;">/</span>
    <span class="font-medium" style="color: #1A0A0A;">{{ $visit->patient?->nama_lengkap ?? 'RME' }}</span>
@endsection
@section('content')
...
<div class="bg-white rounded-2xl p-4 mb-4" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    <div class="flex items-center justify-between gap-4 flex-wrap">
        ...
        @endphp
        <span class="px-3 py-1.5 rounded-full text-xs font-semibold" style="{{ $statusStyle }}">
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}">
        @csrf
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}#asesmen_keperawatan">
        @csrf
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}#asesmen_medis">
        @csrf
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}#resep" id="form-resep">
        @csrf
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}#racikan">
        @csrf
...
@foreach(['penunjang' => ['fa-microscope','Permintaan Penunjang','Pilih jenis penunjang dari menu dropdown'],
         ] as $panelId => $info)
<div id="panel-{{ $panelId }}" class="rme-panel" style="min-width:0;">
<div class="bg-white rounded-2xl p-8 text-center" style="border: 1px solid #E8D5D5;">
    <i class="fa-solid {{ $info[0] }} text-4xl mb-3 block" style="color: #E8D5D5;"></i>
    <p class="text-sm font-semibold" style="color: #1A0A0A;">{{ $info[1] }}</p>
    <p class="text-xs mt-1" style="color: #6B4C4C;">{{ $info[2] }}</p>
</div>
</div>
@endforeach
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}#{{ $panelId }}">
        @csrf
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}#tindakan_dilakukan">
        @csrf
...
<div class="bg-white rounded-2xl p-5" style="border: 1px solid #E8D5D5; box-shadow: 0 2px 8px rgba(123,29,29,0.06);">
    ...
    <form method="POST" action="{{ route('rme.store', $visit->id) }}#resume">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            <div class="md:col-span-2">
                ...
                <textarea name="diagnosis_akhir" rows="3" placeholder="Diagnosis akhir pasien..."
                          class="w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 resize-none"
                          style="border-color:#E8D5D5; --tw-ring-color:rgba(123,29,29,0.15);">{{ old('diagnosis_akhir', $visit->medicalRecord?->assessment) }}</textarea>
            </div>
            <div class="md:col-span-2">
                ...
                <textarea name="tindakan_dilakukan_resume" rows="3" placeholder="Ringkasan tindakan yang telah dilakukan..."
                          class="w-full border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 resize-none"
                          style="border-color:#E8D5D5; --tw-ring-color:rgba(123,29,29,0.15);">{{ old('tindakan_dilakukan_resume', $visit->medicalRecord?->plan) }}</textarea>
            </div>
            <div>
...
function addRacikanRow() {
    var drugs = @json($drugs->map(fn($d) => ['id' => $d->id, 'nama' => $d->nama]));
